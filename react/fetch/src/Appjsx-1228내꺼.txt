...
export default function App() {
  ...
  const fetchUsers = async() => {
    ...
    try{
      ...
      if(!response.ok){  //(ok아니면  404뜸(실패))
        throw new Error('데이트를 불러오는데 실패했습니다.');
      }
      const data = await response.json(); //(제대로 가져오면, javaS 바꿔서)
      // 전달 응답(response) json 객체로 변환하여 data 변수에 저장
      setUsers(data);
    }catch(err){
      setError(err.message);
    }finally{  //(에러상관없이 무조건실행)
      setLoading(false);
    }
  };
  // 컴포넌트 마운트시 데이터 로드  (처음로딩될때)
  useEffect(()=>{
    fetchUsers();  //(한번실행하겠다.)
  },[]);
